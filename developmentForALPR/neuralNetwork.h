...
void processNeuralNetwork(){
	...
	for (int i = 0; i < commPlate.size(); i++)
	{
		...
		cropKana(binaryNP,plateDetectCounter);
		cropBunrui(binaryNP,plateDetectCounter);
		...
		for (int i = 0; i < contours.size(); i++) {
			...
			if ((contourRatio > 0.4  && contourRatio < 0.6 && pointArea[i].height > 30 && pointArea[i].height < 55) || (contourRatio > 0.11 && contourRatio < 0.22 && pointArea[i].height > 30 && pointArea[i].height < 65))
			{
				Rect wideArea = pointArea[i];

				if (wideArea.x <= 3 || wideArea.y <= 3 || (wideArea.x + wideArea.width) >= contoursMat.cols - 6 || (wideArea.y + wideArea.height) >= (contoursMat.rows - 6)) {
					rectangle(contoursMat, pointArea[i], Scalar(0, 255, 0), 1);
					...
					sortA[count] = { wideArea.x, Mat(binaryNP, wideArea) };
				}

				if (contourRatio > 0.11 && contourRatio < 0.22 && pointArea[i].height > 30 && pointArea[i].height < 55 && pointArea[i].y < 60 && pointArea[i].x > 20) {
					if (wideArea.x <= (wideArea.width/2) || wideArea.y <=  0 || wideArea.x + wideArea.width > (contoursMat.cols - wideArea.width)  || (wideArea.y + wideArea.height) >= contoursMat.rows) {
						rectangle(contoursMat, pointArea[i], Scalar(0, 255, 0), 1);
						sortA[count] = { pointArea[i].x, Mat(binaryNP, pointArea[i]) };
					}
					else {
