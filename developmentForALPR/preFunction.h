...
void savePlate(Mat& imageSaved,int& plateDeteCount) {
	if (plateDeteCount % imageSaveRate == 0) {
...
void saveNum(Mat& numSaved, string& strx,int& a) {
	if (plateDetectCounter % imageSaveRate == 0) {
		stringstream savefileName;
...
void cropKana(Mat& resized,int& plateDetCounter) { //binaryNP
	if (plateDetCounter % imageSaveRate == 0) {
		const double kana_rightSide = 0.196;
		const double kana_upperSide = 0.424;
		const double kana_lowerSide = 0.909;

		double matHeight = resized.rows * kana_upperSide;
		double matCols = resized.cols * kana_rightSide;
		double lower = resized.rows * kana_lowerSide;

		Point leftUpper(1, matHeight);
...
void cropBunrui(Mat& resized_bunrui,int& plateDetCouter) {  //binaryNP
	if (plateDetCouter % imageSaveRate == 0) {
		const double bunri_rightSide = 0.833;
		const double bunri_leftSide = 0.181;
		const double bunri_lowerSide = 0.424;

		Point leftUpper((bunri_leftSide * resized_bunrui.cols), 2);
		Point rightLower(bunri_rightSide * resized_bunrui.cols, resized_bunrui.rows * bunri_lowerSide);

		Mat bunrui, area, kubun;
		bunrui = Mat(resized_bunrui, Rect(leftUpper, rightLower));
		const double bunruiCenter = bunrui.cols * 0.495;
		Point bunruiCenterUpper(bunruiCenter, 0);
		area = Mat(bunrui, Rect(Point(0, 0), Point(bunruiCenter, bunrui.rows)));
		kubun = Mat(bunrui, Rect(bunruiCenterUpper, Point(bunrui.cols, bunrui.rows)));
		stringstream saveMatBunrui, saveMatArea, saveMatKubun;
		saveMatBunrui << "./dev/image/upper/" << timestamp() << ".png";
		saveMatArea << "./dev/image/area/" << timestamp() << ".png";
		saveMatKubun << "./dev/image/bunrui/" << timestamp() << ".png";
		imwrite(saveMatBunrui.str(), bunrui);
		imwrite(saveMatArea.str(), area);
		imwrite(saveMatKubun.str(), kubun);
...
void cropArea(Mat& resized,int& plateDetCounter) { //binaryNP
	if (plateDetCounter % imageSaveRate == 0) {
